...
function formatNotice(n: HilmaNotice): string {
  ...
  if (n.nutsCodes && n.nutsCodes.length > 0)
    lines.push(`**Alue (NUTS):** ${Array.isArray(n.nutsCodes) ? n.nutsCodes.join(", ") : n.nutsCodes}`);
  if (n.estimatedValue)
    lines.push(
...
async function getNoticeFullXml(noticeId: number): Promise<string> {
  ...
  // ── Perustiedot ───────────────────────────────────────────────
  // Otetaan ensimmäinen nimi (FIN)
  const title = first(/<cac:ProcurementProject>[\s\S]*?<cbc:Name[^>]*>([^<]+)<\/cbc:Name>/i)
             ?? first(/<cbc:Name languageID="FIN">([^<]+)<\/cbc:Name>/i)
             ?? "–";

  // Kuvaus — ensimmäinen <cbc:Description> ProcurementProject-lohkossa
  const descMatch = xml.match(/<cac:ProcurementProject>[\s\S]*?<cbc:Description[^>]*>([\s\S]*?)<\/cbc:Description>/i);
  const description = descMatch ? descMatch[1].trim() : null;

  // CPV pääkoodi
  const cpvCodes = all(/<cac:MainCommodityClassification>[\s\S]*?<cbc:ItemClassificationCode[^>]*>([^<]+)<\/cbc:ItemClassificationCode>/gi);
  const cpvAdd   = all(/<cac:AdditionalCommodityClassification>[\s\S]*?<cbc:ItemClassificationCode[^>]*>([^<]+)<\/cbc:ItemClassificationCode>/gi);

  // NUTS — tilaajan alue (FI1D...)
  const nutsAll = all(/<cbc:CountrySubentityCode listName="nuts">([^<]+)<\/cbc:CountrySubentityCode>/gi);
  const nuts = [...new Set(nutsAll)];

  // Deadlinet — tarjousten jättöaika (suurin EndDate ennen sopimuksen alkua)
  const allEndDates = all(/<cac:TenderSubmissionDeadlinePeriod>[\s\S]*?<cbc:EndDate>([^<]+)<\/cbc:EndDate>/gi);
  const allEndTimes = all(/<cac:TenderSubmissionDeadlinePeriod>[\s\S]*?<cbc:EndTime>([^<]+)<\/cbc:EndTime>/gi);
  const tenderDeadline = allEndDates[0] ?? null;
  const tenderDeadlineTime = allEndTimes[0] ?? null;

  // Kysymysten deadline (toinen EndDate jos on)
  const questionDeadline = allEndDates[1] ?? null;
  const questionDeadlineTime = allEndTimes[1] ?? null;

  // Sopimuskausi — PlannedPeriod tai ContractExecutionRequirement
  const contractStart = first(/<cac:PlannedPeriod>[\s\S]*?<cbc:StartDate>(\d{4}-\d{2}-\d{2})/i);
  // EndDate sopimuskaudelle — etsitään kauimpana tulevaisuudessa (ei deadline)
  const allDatesInXml = all(/<cbc:EndDate>(\d{4}-\d{2}-\d{2})/gi);
  const tenderDeadlineDate = tenderDeadline ? tenderDeadline.substring(0, 10) : null;
  const contractEndCandidates = allDatesInXml.filter(d => d !== tenderDeadlineDate && d > "2026-06-01");
  const contractEnd = contractEndCandidates.sort().pop() ?? null;

  // Arvioitu arvo
  const estimatedValue = first(/<cbc:EstimatedOverallContractAmount[^>]*currencyID="([A-Z]+)"[^>]*>([^<]+)<\/cbc:EstimatedOverallContractAmount>/i)
                      ?? null;
  const estimatedAmount = xml.match(/<cbc:EstimatedOverallContractAmount[^>]*>([^<]+)<\/cbc:EstimatedOverallContractAmount>/i)?.[1] ?? null;
  const estimatedCurrency = xml.match(/<cbc:EstimatedOverallContractAmount[^>]*currencyID="([A-Z]+)"/i)?.[1] ?? "EUR";

  // Menettely
  const procCode = first(/<cbc:ProcedureCode[^>]*>([^<]+)<\/cbc:ProcedureCode>/i);

  // Hankintanumero (Hilma identifier)
  const hankintaNro = first(/<hilma:Identifier>([^<]+)<\/hilma:Identifier>/i)
                   ?? first(/<efbc:Identifier>([^<]+)<\/efbc:Identifier>/i);

  // ── Yhteystiedot — hankintayksikkö (ei valitustuomioistuin) ───
  const EXCLUDE_EMAILS = new Set(["markkinaoikeus@oikeus.fi", "tekninen@hankintailmoitukset.fi"]);

  // Poimi Contact-lohkot koko XML:stä
  const contactBlocks = [...xml.matchAll(/<cac:Contact>([\s\S]*?)<\/cac:Contact>/gi)].map(m => m[1]);
  const contacts: { name: string | null; email: string | null; phone: string | null }[] = [];
  for (const block of contactBlocks) {
    const email = block.match(/<cbc:ElectronicMail>([^<]+)<\/cbc:ElectronicMail>/i)?.[1]?.trim() ?? null;
    if (email && EXCLUDE_EMAILS.has(email)) continue;
    const name  = block.match(/<cbc:Name>([^<]+)<\/cbc:Name>/i)?.[1]?.trim() ?? null;
    const phone = block.match(/<cbc:Telephone>([^<]+)<\/cbc:Telephone>/i)?.[1]?.trim() ?? null;
    if (email || name || phone) contacts.push({ name, email, phone });
  }

  // ── Tarjousportaalin URL:t ─────────────────────────────────────
  const urlMatches = [...xml.matchAll(/<cbc:URI>([^<]+)<\/cbc:URI>/g)].map(m => m[1]).filter(u => u.startsWith("http"));
  const uniqueUrls = [...new Set(urlMatches)];

  // ── Arviointikriteerit ─────────────────────────────────────────
  const subCriteria: string[] = [];
  const subBlocks = [...xml.matchAll(/<cac:SubordinateAwardingCriterion>([\s\S]*?)<\/cac:SubordinateAwardingCriterion>/gi)];
  for (const b of subBlocks) {
    const sname   = b[1].match(/<cbc:Name[^>]*>([^<]+)<\/cbc:Name>/i)?.[1]?.trim();
    const sweight = b[1].match(/<efbc:AwardCriterionNumber[^>]*>([^<]+)<\/efbc:AwardCriterionNumber>/i)?.[1]?.trim();
    if (sname) subCriteria.push(sweight ? `${sname} — ${sweight}%` : sname);
  }
  if (subCriteria.length === 0) {
    const mainType = first(/<cbc:AwardingCriterionTypeCode[^>]*>([^<]+)<\/cbc:AwardingCriterionTypeCode>/i);
    if (mainType) subCriteria.push(mainType);
  }

  // ── Rakenna vastaus ───────────────────────────────────────────
  const lines: string[] = [
    `## Ilmoitus ${noticeId} — täydet tiedot`,
    "",
    `**Nimi:** ${title}`,
  ];

  if (hankintaNro) lines.push(`**Hankintanumero:** ${hankintaNro}`);
  lines.push(`**Hilma:** https://hankintailmoitukset.fi/fi/notice/${data.id}`);
  lines.push("");

  // Perustiedot
  const cpvStr = [...new Set([...cpvCodes, ...cpvAdd])].join(", ") || "–";
  const nutsStr = nuts.join(", ") || "–";
  lines.push(`**CPV:** ${cpvStr}`);
  lines.push(`**NUTS (alue):** ${nutsStr}`);
  if (procCode) lines.push(`**Menettely:** ${procCode}`);
  if (estimatedAmount) lines.push(`**Arvioitu arvo:** ${estimatedAmount} ${estimatedCurrency}`);
  lines.push("");

  // Aikataulu
  lines.push("### Aikataulu");
  if (tenderDeadline) {
    const time = tenderDeadlineTime ? ` klo ${tenderDeadlineTime.replace(/\+.*|Z/, "").substring(0,5)}` : "";
    lines.push(`**Tarjousten deadline:** ${tenderDeadline.replace(/\+.*|Z/, "")}${time}`);
  }
  if (questionDeadline) {
    const time = questionDeadlineTime ? ` klo ${questionDeadlineTime.replace(/\+.*|Z/, "").substring(0,5)}` : "";
    lines.push(`**Kysymysten deadline:** ${questionDeadline.replace(/\+.*|Z/, "")}${time}`);
  }
  if (contractStart) lines.push(`**Sopimus alkaa:** ${contractStart.replace(/\+.*|Z/, "")}`);
  if (contractEnd)   lines.push(`**Sopimus päättyy:** ${contractEnd}`);
  lines.push("");

  // Arviointikriteerit
  if (subCriteria.length > 0) {
    lines.push("### Arviointikriteerit");
    subCriteria.forEach(c => lines.push(`- ${c}`));
    lines.push("");
  }

  // Kuvaus
  if (description) {
    lines.push("### Hankkeen kuvaus");
    lines.push(description.substring(0, 3000) + (description.length > 3000 ? "\n\n*(kuvaus jatkuu — katso portaali)*" : ""));
    lines.push("");
  }
  ...
  if (contacts.length > 0) {
    ...
    for (const c of contacts) {
      if (c.name)  lines.push(`**Nimi:** ${c.name}`);
      if (c.email) lines.push(`**Sähköposti:** ${c.email}`);
      if (c.phone) lines.push(`**Puhelin:** ${c.phone}`);
      lines.push("");
    }
  }
  ...
  if (uniqueUrls.length > 0) {
    lines.push("### Tarjouspyyntöportaali");
    uniqueUrls.slice(0, 3).forEach(u => lines.push(`- ${u}`));
  }
