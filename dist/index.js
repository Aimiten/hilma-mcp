...
const HILMA_NOTICE_URL = "https://api.hankintailmoitukset.fi/avp/eformnotices/docs";
...
function buildFilter(params) {
    ...
    if (params.cpv_codes && params.cpv_codes.length > 0) {
        const cpvParts = params.cpv_codes.map((code) => `search.ismatch('${code}', 'cpvCodes')`);
        parts.push(cpvParts.length === 1 ? cpvParts[0] : `(${cpvParts.join(" or ")})`);
    }
...
function formatNotice(n) {
    ...
    if (n.datePublished)
        lines.push(`**Julkaistu:** ${new Date(n.datePublished).toLocaleDateString("fi-FI")}`);
    if (n.expirationDate)
        lines.push(`**Tarjousaika päättyy:** ${new Date(n.expirationDate).toLocaleDateString("fi-FI")}`);
    if (n.mainType)
        lines.push(`**Tyyppi:** ${n.mainType}`);
    if (n.procurementTypeCode)
        lines.push(`**Hankintalaji:** ${n.procurementTypeCode}`);
    if (n.procedureType)
        lines.push(`**Menettely:** ${n.procedureType}`);
    if (n.cpvCodes)
        lines.push(`**CPV-koodit:** ${n.cpvCodes}`);
    if (n.nutsCodes && n.nutsCodes.length > 0)
        lines.push(`**Alue (NUTS):** ${n.nutsCodes.join(", ")}`);
    if (n.estimatedValue)
        lines.push(`**Arvioitu arvo:** ${n.estimatedValue.toLocaleString("fi-FI")} €`);
    if (n.descriptionFi) {
        const desc = n.descriptionFi.length > 500
            ? n.descriptionFi.slice(0, 500) + "…"
            : n.descriptionFi;
        lines.push(`**Kuvaus:** ${desc}`);
    }
    lines.push(`**Linkki:** https://hankintailmoitukset.fi/fi/notice/${n.eFormsId ?? n.noticeId}`);
    return lines.join("\n");
}
// ---------------------------------------------------------------------------
// API calls
// ---------------------------------------------------------------------------
async function searchNotices(params) {
...
async function getNotice(noticeId) {
    ...
    if (!res.ok) {
        throw new Error(`Hilma API error: ${res.status} ${res.statusText} — ilmoitusta ${noticeId} ei löydy tai API-avain ei ole voimassa.`);
    }
...
server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "search_notices",
                description: "Hae hankintailmoituksia Hilmasta (hankintailmoitukset.fi). " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        search: {
                            type: "string",
                            description: 'Vapaatekstihaku ilmoituksen otsikosta ja kuvauksesta. Käytä "*" kaikkien hakemiseen.',
                        },
                        cpv_codes: {
                            type: "array",
                            items: { type: "string" },
                            description: 'CPV-koodit suodatukseen, esim. ["71200000", "72000000"]. Useampi koodi OR-logiikalla.',
                        },
                        notice_type: {
                            type: "string",
                            enum: ["ContractNotices", "ContractAwardNotices", "PlanNotices"],
                            description: "Ilmoitustyyppi: ContractNotices=hankintailmoitukset, ContractAwardNotices=jälki-ilmoitukset, PlanNotices=ennakkoilmoitukset",
                        },
                        procurement_type: {
                            type: "string",
                        ...
                        procedure_type: {
                            ...
                            description: "Menettelytyyppi: open=avoin, restricted=rajoitettu, negotiated=neuvottelu",
                        },
                        days: {
                            type: "number",
                        ...
                        order_by: {
                            type: "string",
                            description: 'Lajittelujärjestys, esim. "datePublished desc" tai "expirationDate asc"',
                        },
                    },
                },
            },
            {
                name: "get_notice",
                description: "Hae yksittäisen hankintailmoituksen täydet tiedot Hilmasta ilmoituksen ID:llä (noticeId). " +
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "search_notices") {
            const result = await searchNotices(args);
